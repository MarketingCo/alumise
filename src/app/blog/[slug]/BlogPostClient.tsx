...
export default function BlogPostClient({ slug }: { slug: string }) {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Blog Hero */}
      <section className="relative h-[60vh] min-h-[500px] flex items-end bg-alumise-obsidian text-white overflow-hidden">
        <Image 
          src={post.image}
          alt={post.title}
          fill
        ...
        <div className="container mx-auto px-4 max-w-4xl relative z-10 pb-20">
          ...
          <div className="flex items-center space-x-4 text-[10px] uppercase tracking-widest font-bold text-alumise-gold mb-6">
            <span className="bg-alumise-gold/20 backdrop-blur-md px-3 py-1 border border-alumise-gold/30 text-white">{post.category}</span>
          ...
          <div className="bg-gray-50 border border-gray-100 p-12 my-20 rounded-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <Link 
                  href="/quote" 
                  className="inline-flex items-center bg-alumise-obsidian text-white px-8 py-4 rounded-sm text-[10px] font-bold uppercase tracking-widest hover:bg-alumise-gold transition-all"
                >
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center text-xs font-bold uppercase tracking-tight">
                  <Zap className="w-4 h-4 mr-3 text-alumise-gold" /> Precision Engineering
                </div>
                <div className="flex items-center text-xs font-bold uppercase tracking-tight">
                  <CheckCircle2 className="w-4 h-4 mr-3 text-alumise-gold" /> Certified Installation
                </div>
              </div>
            </div>
          </div>
          ...
          <footer className="pt-20 border-t border-gray-100">
            <div className="flex justify-between items-center">
              ...
              <div className="flex space-x-4">
                ...
                <div className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 hover:text-alumise-gold transition-colors cursor-pointer">
      ...
      <section className="py-24 bg-gray-50 border-t border-gray-100">
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-alumise-gold mb-4">Deep Learning</h2>
              <p className="text-3xl font-bold uppercase tracking-tight">Related <span className="text-gray-600">Insights.</span></p>
            </div>
            <Link href="/blog" className="text-xs font-bold uppercase tracking-widest border-b border-alumise-obsidian pb-1 hover:text-alumise-gold transition-all">
              All Articles
            </Link>
          </div>
            ...
            {blogPosts
              ...
              .map((p) => (
                <Link key={p.slug} href={`/blog/${p.slug}`} className="group block bg-white border border-gray-100 p-8 hover:shadow-xl transition-all">
                  <div className="text-[8px] uppercase tracking-widest font-bold text-alumise-gold mb-4">{p.category}</div>
                  <h4 className="font-bold uppercase tracking-tight text-lg mb-4 group-hover:text-alumise-gold transition-colors leading-tight">{p.title}</h4>
