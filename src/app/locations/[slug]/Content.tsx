...
export default function LocationContent({ slug }: { slug: string }) {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <section className="bg-brand-charcoal pt-40 pb-24 text-white relative overflow-hidden">
        <div className="absolute top-0 right-0 w-1/3 h-full bg-brand-gold opacity-5 blur-3xl rounded-full translate-x-1/2"></div>
        <div className="container mx-auto px-4 max-w-7xl relative z-10">
          ...
          <h1 className="text-5xl md:text-8xl font-bold uppercase mb-8 tracking-tighter max-w-4xl">
            Architectural Glazing in <span className="text-brand-gold">{location.name}.</span>
          </h1>
      ...
      <section className="py-24 border-b border-gray-50">
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="text-center mb-16 space-y-4">
            <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-brand-gold">Regional Expertise</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {categories.map((cat, idx) => (
              <div key={cat} className="p-8 border border-gray-100 bg-white hover:shadow-xl transition-all">
                <Building2 className="w-8 h-8 text-brand-gold mb-6" />
        ...
        <div className="container mx-auto px-4 max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="space-y-12">
            <div className="space-y-6">
              <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-brand-gold">Total Clarity</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div className="flex items-start">
                <ShieldCheck className="w-5 h-5 mr-4 text-brand-gold flex-shrink-0" />
                <span className="text-sm font-bold uppercase tracking-tight">FENSA Accredited</span>
              </div>
              <div className="flex items-start">
                <Zap className="w-5 h-5 mr-4 text-brand-gold flex-shrink-0" />
                <span className="text-sm font-bold uppercase tracking-tight">High Thermal Efficiency</span>
              </div>
              <div className="flex items-start">
                <Ruler className="w-5 h-5 mr-4 text-brand-gold flex-shrink-0" />
          ...
          <div className="container mx-auto px-4 max-w-7xl">
            <div className="flex justify-between items-end mb-16">
              <div className="space-y-4">
                <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-brand-gold">Proven Locally</h2>
                <p className="text-3xl font-bold uppercase tracking-tight">Case Studies in {location.name}</p>
              </div>
              <Link href="/projects" className="text-xs font-bold uppercase tracking-widest border-b border-brand-charcoal pb-1 hover:text-brand-gold transition-all">
                View All Projects
              </Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {localProjects.map((project) => (
                <Link key={project.slug} href={`/projects/${project.slug}`} className="group bg-white border border-gray-100 overflow-hidden hover:shadow-2xl transition-all">
                  ...
                  <div className="p-8">
                    <span className="text-[10px] uppercase tracking-widest font-bold text-brand-gold block mb-2">{project.type}</span>
                    <h4 className="text-lg font-bold uppercase tracking-tight mb-4 group-hover:text-brand-gold transition-colors">{project.title}</h4>
      ...
      <section className="py-40 bg-white text-center">
        <div className="container mx-auto px-4 max-w-3xl">
          <h2 className="text-5xl font-bold uppercase mb-8 tracking-tighter">
            Discuss Your Project in <span className="text-brand-gold">{location.name}.</span>
          </h2>
